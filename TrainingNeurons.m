...
%Reference = imread('PV ChR2 Animal 5.tif');

%improtcrop = imcrop(improt, [size(improt,1)/7,size(improt,2)/3,size(improt,1)/2,size(improt,2)/4]);

%User Defined Area Reference
%Create 3 images 

filepath = '/Users/SD/Documents/Matlab/High Pass Filter/Reference';

%{
for i=1:3
    [CropReference, rect] =imcrop(Reference);
    imwrite(CropReference, [filepath '/Reference#' num2str(i) '.tif']);
end
close(gcf);
%}
